document.addEventListener("DOMContentLoaded", () => {
  ...
  function addLocalPizzaOptions() {
    ...
    // Replace any existing items in these categories with the configured set
    ...
    state.menu = state.menu.filter(
      (m) => !categoriesToReplace.includes(m.category)
    );

    const configuredItems = [];

    // Crusts
    configuredItems.push({ name: "Thin", category: "Crust", price: 0.0 });
    configuredItems.push({ name: "Regular", category: "Crust", price: 0.0 });
    configuredItems.push({ name: "Pan", category: "Crust", price: 1.5 });

    // Sauces
    configuredItems.push({ name: "Marinara", category: "Sauce", price: 0.5 });
    configuredItems.push({ name: "Alfredo", category: "Sauce", price: 0.8 });
    ...
    toppings.forEach((t) =>
      configuredItems.push({
        name: t,
        category: "Topping",
        price: t === "Cheese" ? 0.0 : 1.0,
      })
    );

    // Beverages
    configuredItems.push({ name: "Coke", category: "Drinks", price: 1.99 });
    configuredItems.push({ name: "Sprite", category: "Drinks", price: 1.99 });
    configuredItems.push({ name: "Fanta", category: "Drinks", price: 1.99 });
    ...
    configuredItems.forEach((ci) => {
      const newItem = Object.assign({ id: _localMenuId }, ci);
      _localMenuId -= 1;
      ...
      if (categoryName === "Drinks" || categoryName === "Desserts") {
        ...
        card.innerHTML = `
                    ...
                    <div class="item-qty-controls">
                        <button type="button" class="btn btn-primary btn-qty-plus" data-item-id="${
                          item.id
                        }">+</button>
                        <span id="qty-${
                          item.id
                        }" class="item-qty">${currentQty}</span>
                        <button type="button" class="btn btn-secondary btn-qty-minus" data-item-id="${
                          item.id
                        }">-</button>
                    </div>
                `;
  ...
  function handlePizzaSubmit(event) {
    ...
    customizations.push(
      ...customizations.filter((c) => c.category === "Topping")
    );
  ...
  function renderCheckoutScreen() {
    ...
    state.cart.forEach((item) => {
      ...
      itemEl.innerHTML = `<span>${item.quantity}x ${
        item.name
  ...
  async function handleCheckout(event) {
    ...
    if (paymentType === "Card") {
      ...
      if (!signature || signature.trim().length < 2) {
        showMessage(
          messages.checkout,
          "Signature required for card payments.",
          "error"
        );
        return;
      }
  ...
  function renderEmployeeDashboard() {
    ...
    document.querySelector(
      "#employee-screen h2"
    ).textContent = `Employee Portal - ${state.currentUser.first_name}`;

    if (btnIn)
      btnIn.onclick = () => {
        state.clockedIn = true;
        status.textContent =
          "Status: Clocked In at " + new Date().toLocaleTimeString();
        btnIn.disabled = true;
    ...
    if (btnOut)
      btnOut.onclick = () => {
        ...
        status.textContent =
          "Status: Clocked Out at " + new Date().toLocaleTimeString();
  ...
  function renderManagerDashboard() {
    ...
    if (invList)
      invList.innerHTML = mockDB.inventory
        .map(
          (i) =>
    ...
    if (empList)
      empList.innerHTML = mockDB.employees
        .map(
          (e) =>
    ...
    window.filterSales = (period) => {
      if (salesList)
        salesList.innerHTML = mockDB.sales
          .map(
            (s) =>
              `<div class="data-row"><span>${
                s.items
              }</span><span>$${s.total.toFixed(2)}</span></div>`
          )
          .join("");
    };
