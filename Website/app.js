...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function handleLogin(event) {
        ...
        try {
            ...
            showMessage(messages.login, "Connection error. Please try again.", "error");
        }
    }
    ...
    async function handleSignup(event) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showMessage(messages.signup, "Account already exists with this phone number.", "error");
            }
        } catch (error) {
            console.error("Signup error:", error);
            showMessage(messages.signup, "Connection error. Please try again.", "error");
        }
    }
    ...
    function renderCategoryScreen(categoryName) {
        ...
        items.forEach(item => {
            ...
            card.innerHTML = `
                ...
                <button class="btn btn-primary btn-add-item" data-item-id="${item.id}">Add</button>
            `;
    ...
    async function handleCheckout(event) {
        ...
        if (cardNum.length < 15 || cardExp.length < 4 || cardCvv.length < 3 || cardZip.length < 5) {
